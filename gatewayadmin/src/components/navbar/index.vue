  ...
  <div class="navbar">
    <div class="left-side">
      <a-space>
        ...
        <a-typography-title :heading="5" :style="{ margin: 0, fontSize: '18px' }">Ocelot Admin</a-typography-title>
    ...
    <ul class="right-side">
      <li>
        <a-tooltip :content="$t('settings.language')">
          <a-button :shape="'circle'" class="nav-btn" type="outline" @click="setDropDownVisible">
            <template #icon>
              <icon-language />
            </template>
          </a-button>
        ...
        <a-dropdown trigger="click" @select="changeLocale as any">
          ...
          <template #content>
            <a-doption v-for="item in locales" :key="item.value" :value="item.value">{{ item.label }}</a-doption>
          </template>
        </a-dropdown>
      </li>
      <li>
